  ...
  return (
    <section className="pt-20 pb-16 bg-card">
      ...
      <div className="mx-auto max-w-5xl px-6 mt-16">
        <div className="gsap-stagger grid grid-cols-2 md:grid-cols-3 gap-4">
          {vendorPlaceholders.slice(0, 6).map((vendor) => (
            <div 
              key={vendor.id} 
              className="gsap-stagger-item group border border-border/50 bg-foreground/5 aspect-[2.5/1] flex items-center justify-center transition-all duration-300 hover:border-accent/50 hover:bg-foreground/10"
            >
        ...
        <p className="gsap-fade-in mt-10 text-xs text-muted-foreground/60 text-center">
          Vendor list shown for reference purposes only. No authorization or partnership is implied.
        </p>
        
        <div className="gsap-fade-up mt-8 text-center">
          <Link 
            href="/vendors" 
